        ...
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            ...
            for idx, table in enumerate(all_tables):
                ...
                for row in table[1:]:  # Пропускаем заголовок
                    ...
                    # Обычно в строке 3 числа: публикации, цитирования, Хирш
...
def init_db():
    ...
    # Таблица публикаций с привязкой к году
    cursor.execute('''
...
def save_to_database(data, year):
    ...
    for item in data:
        ...
        if emp_result:
            ...
            # Обновляем h_index (берём максимальный)
        ...
        # Сохраняем данные за год (публикации и цитирования)
        cursor.execute('''
...
def get_departments():
    """Получение данных с фильтром по году"""
    ...
    if year_filter == 'all':
        # Без фильтра — суммируем по всем годам
        cursor.execute('''
            SELECT 
                s.id, s.name,
